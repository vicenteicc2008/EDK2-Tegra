...
#include <Library/EarlyDisplayInitLib.h>
...
SerialPortInitialize(VOID)
{
  ...
  LocateMemoryMapAreaByName("Display", &DisplayMemoryRegion);
...
void ResetFb(void)
{
  ...
  UINTN BgColor = FB_RGB888_BLACK;

  // Set to black color.
  for (UINTN i = 0; i < gWidth; i++) {
    for (UINTN j = 0; j < gHeight; j++) {
      BgColor = FB_RGB888_BLACK;
      // Set pixel bit
...
void FbConReset(void)
{
  ...
  // Reset color.
  m_Color.Foreground = FB_RGB888_WHITE;
  m_Color.Background = FB_RGB888_BLACK;
}
...
SerialPortWriteCritical(IN UINT8 *Buffer, IN UINTN NumberOfBytes)
{
  ...
  m_Color.Foreground = FB_RGB888_YELLOW;
