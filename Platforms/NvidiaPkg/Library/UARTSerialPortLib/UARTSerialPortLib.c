        ...
        if(sizeof(RxBuffer) - WriteIndex > 4) {
            ...
            if(WriteOffset + AvailBytes > sizeof(RxBuffer) - WriteIndex) {
                if(sizeof(RxBuffer) - WriteIndex - WriteOffset < 4) {
                    ...
                    BytesRead = 0;
                    AvailBytes = WordsRead * 4;
                }
            } else {
                MemWrite(GENI4_DATA, GENI_S_IRQ_CLEAR_ADDR, (IRQStatus&RX_LAST_IRQ));
            }
            ...
            for(Index = 0; Index < WordsRead; Index++) {
                ...
                if(Status) {
                    ReadBuffer[0] = (UINT8)(Word >> 0);
                    ReadBuffer[1] = (UINT8)(Word >> 8);
                    ReadBuffer[2] = (UINT8)(Word >> 16);
                    ReadBuffer[3] = (UINT8)(Word >> 24);
                } else {
                    *(UINT32*)ReadBuffer = Word;
                }
                ReadBuffer += 4;
            }
            ...
            if(BytesRead) {
                ...
                for(Index = 0; Index < BytesRead; Index++) {
                    ReadBuffer[Index] = (UINT8)(Word >> Index * 8);
                }
...
okay:
                ...
                for(Index=0; Index<Num; Index++) {
                    *Buff++ = RxBuffer[ReadIndex++ & (sizeof(RxBuffer) - 1)];
                }
  ...
  IN UINTN Bytes)
{
    ...
    if(TxFIFOSize == 0) {
        TxFIFOSize = FIFOSize << 2;
    }

    while(BytesRemain > 0) {
        Send = (BytesRemain > TxFIFOSize) ? TxFIFOSize : BytesRemain;

        if(MemReadMask(GENI4_CFG, GENI_STATUS) & M_GENI_CMD_ACTIVE) {
            continue;
        }
        ...
        if(Send) {
            ...
            WordsSend = Full > Slots ? Slots : Full;
            BytesSend = Full >= Slots ? 0 : Partial;
            BOOLEAN k = ((CHAR8*)Buffer - (CHAR8*)NULL) & 0x03;

            for(Index = 0; Index < WordsSend; Index++) {
                MemWrite(GENI4_DATA, GENI_TX_FIFO_ADDR, (k ? (Buffer[0] << 0 | Buffer[1] << 8 | Buffer[2] << 16 | Buffer[3] << 24) : (*(UINT32*) Buffer)));
                Buffer += 4;
            }
            ...
            if(BytesSend) {
                ...
                for(Index = 0;Index < BytesSend; Index++) {
                    WordValue |= Buffer[Index] << Index * 8;
                }
                MemWrite(GENI4_DATA, GENI_TX_FIFO_ADDR, WordValue);
            }
            MemWrite(GENI4_DATA, (Send == WordsSend * 4 + BytesSend) ? GENI_M_IRQ_EN_CLEAR_ADDR : GENI_M_IRQ_EN_SET_ADDR, TX_FIFO_WATERMARK_IRQ);
        }
        MsgBuff += Send;
        BytesRemain -= Send;
    }
...
SerialPortPoll(VOID)
{
    return (MemReadMask(GENI4_DATA, GENI_S_IRQ_STATUS) & (RX_LAST_IRQ | RX_FIFO_WATERMARK_IRQ)) || (WriteIndex - ReadIndex > 0);
}
